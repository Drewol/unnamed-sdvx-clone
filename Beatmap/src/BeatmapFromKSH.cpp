	...
	for (KShootMap::TickIterator it(kshootMap); it; ++it)
	{
		...
		for (auto &p : tick.settings)
		{
			...
			else if (p.first == "zoom_bottom")
			{
				...
				m_baseEffects.InsertGraphValue(EffectTimeline::GraphType::ZOOM_BOTTOM, mapTime, value);
			}
			else if (p.first == "zoom_top")
			{
				const double value = atol(p.second.data()) / 100.0;
				m_baseEffects.InsertGraphValue(EffectTimeline::GraphType::ZOOM_TOP, mapTime, value);
			}
			else if (p.first == "zoom_side")
			{
				const double value = atol(p.second.data()) / 100.0;
				m_baseEffects.InsertGraphValue(EffectTimeline::GraphType::SHIFT_X, mapTime, value);
			}
			...
			*/
			...
			else if (p.first == "tilt")
			{
				...
				else
				{
					...
					m_baseEffects.InsertGraphValue(EffectTimeline::GraphType::ROTATION_Z, mapTime, rotation);
				...
				if (isManualTilt)
				{
					m_baseEffects.GetGraph(EffectTimeline::GraphType::ROTATION_Z).Extend(mapTime);
				}
